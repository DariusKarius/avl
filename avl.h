...
public:
    ...
    T if_not_found_succesor(T value){
        return if_not_found_succesor(root, value);
    }

    T if_not_found_predecesor(T value){
        return if_not_found_predecesor(root, value);
    }

    string search_by_range(T begin, T end){
        if(find(begin) == false)
...
private:
    ...
    string search_by_range(NodeBT<T>* node, T begin, T end);
    T if_not_found_succesor(NodeBT<T>* node, int data);
    T if_not_found_predecesor(NodeBT<T>* node, int data);
};
...
template <typename T>
string AVLTree<T>::search_by_range(NodeBT<T>* node, T begin, T end) {
    string result = "";
    T n = begin;
...
template <typename T>
T AVLTree<T>::if_not_found_succesor(NodeBT<T> *node, int value) {
    T n;
    stack<T> mstack;
    while(node!= nullptr) {
        if (node->data > value) {
...
template <typename T>
T AVLTree<T>::if_not_found_predecesor(NodeBT<T> *node, int value) {
    T n;
    queue<T> mqueue;
    while(node!= nullptr) {
        if (node->data > value) {
...
template <typename T>
T AVLTree<T>::successor(NodeBT<T> *node, T value) {
    stack<T> mstack;
    ...
    if(node->data==maxValue())
        throw runtime_error("Rango incorrecto");
    else if(node->right!= nullptr){
        return minValue_wp(node->right);
    }
    else{
        return mstack.top();
    }
